...
import { DEFAULT_SETTINGS, DARK_THEME_SETTINGS, type ThemeSettings } from '../config/themeSettings';
 ...
 */
function positionsToBoxShadow(positions: Array<{x: number, y: number, size: number, withGlow: boolean}>, color: string): string {
  const stars: string[] = [];
  for (const pos of positions) {
    if (pos.withGlow) {
      // Add glow effect with 1px offset
      stars.push(`${pos.x}px ${pos.y}px 0px ${pos.size}px ${color}, ${pos.x + 1}px ${pos.y + 1}px ${pos.size * 4}px ${pos.size}px ${color}`);
    } else {
...
export const Stars = memo(function Stars({ isDark = true, settings }: StarsProps) {
  ...
  const largeStarPositions = useMemo(() => {
    return generateStarPositions(8, 0.82, false, 0, 100);
  }, []); // Empty deps - positions generated once and never change
  ...
  const starsBig = useMemo(() => {
    const color = isDark ? darkStarConfigs.large.color : lightStarConfigs.large.color;
    return positionsToBoxShadow(largeStarPositions, color);
  }, [isDark, darkStarConfigs.large.color, lightStarConfigs.large.color, largeStarPositions]);
