...
export const DeploymentStatus = React.memo<DeploymentStatusProps>(({
  ...
  const filteredWorkflows = useMemo(() => {
    ...
    return workflows.filter(w => {
      ...
      if (selectedFilter === 'deploy-database') return name.includes('database') || name.includes('db');
  ...
  const getStatusBadge = (status: WorkflowRun['status'], conclusion?: string) => {
    if (status === 'in_progress') {
      return (
        <Badge variant="info" className="flex items-center gap-1 bg-blue-100 text-blue-800 border border-blue-300">
    ...
    if (status === 'completed') {
      if (conclusion === 'success') {
        return (
          <Badge variant="success" className="flex items-center gap-1 bg-emerald-100 text-emerald-800 border border-emerald-300">
      ...
      if (conclusion === 'failure') {
        return (
          <Badge variant="danger" className="flex items-center gap-1 bg-rose-100 text-rose-800 border border-rose-300">
    ...
    if (status === 'cancelled') {
      return (
        <Badge variant="default" className="flex items-center gap-1 bg-slate-100 text-slate-800 border border-slate-300">
    ...
    return (
      <Badge variant="warning" className="flex items-center gap-1 bg-amber-100 text-amber-800 border border-amber-300">
  ...
  return (
    <VisionCard className={className} isDark={isDark} noPadding>
      {/* Header */}
      <div className={`flex items-center justify-between p-5 border-b ${isDark ? 'border-white/10' : 'border-slate-300/50'}`}>
        ...
        >
          <RefreshCw className={`w-5 h-5 ${isDark ? 'text-slate-300' : 'text-slate-700'} ${(isLoading || isRefreshing) ? 'animate-spin' : ''}`} />
        </button>
      </div>

      {/* Metrics Grid */}
      {metrics && (
        <div className={`grid grid-cols-4 gap-4 p-5 border-b ${isDark ? 'border-white/10' : 'border-slate-300/50'}`}>
          {/* Average Build Time */}
      ...
      {/* Filter Bar */}
      <div className={`flex items-center gap-2 p-4 border-b ${isDark ? 'border-white/10' : 'border-slate-300/50'}`}>
      ...
      <div className="p-5 space-y-3 max-h-[600px] overflow-y-auto custom-scrollbar">
        {isLoading && workflows.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-12">
            <Loader2 className={`w-8 h-8 animate-spin ${isDark ? 'text-slate-400' : 'text-slate-600'}`} />
          ...
          <div className="flex flex-col items-center justify-center py-12">
            <Activity className={`w-8 h-8 ${isDark ? 'text-slate-400' : 'text-slate-600'} opacity-50`} />
            <p className={`mt-3 text-sm ${isDark ? 'text-slate-400' : 'text-slate-600'}`}>
                ...
                <div className="flex gap-4 flex-1 min-w-0">
                  ...
                  <div className="flex flex-col items-center pt-1">
                    ...
                    {index < filteredWorkflows.length - 1 && (
                      <div className={`w-0.5 h-full min-h-[60px] mt-2 ${isDark ? 'bg-white/10' : 'bg-slate-300'}`} />
                    )}
                  </div>
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-start justify-between gap-2 mb-2">
                      ...
                      <ExternalLink className={`w-4 h-4 flex-shrink-0 opacity-0 group-hover:opacity-100 transition-opacity ${isDark ? 'text-slate-400' : 'text-slate-600'}`} />
                    </div>

                    {/* Metadata */}
                    <div className="space-y-1.5">
                      <div className="flex items-center gap-2">
                        <GitBranch className={`w-3.5 h-3.5 ${isDark ? 'text-slate-400' : 'text-slate-600'}`} />
                      ...
                      <div className="flex items-center gap-2">
                        <GitCommit className={`w-3.5 h-3.5 ${isDark ? 'text-slate-400' : 'text-slate-600'}`} />
                        <span className={`text-xs font-mono ${isDark ? 'text-slate-300' : 'text-slate-700'}`}>
                          {workflow.commit_sha.substring(0, 7)}
                        </span>
                        <span className={`text-xs ${isDark ? 'text-slate-400' : 'text-slate-600'} truncate`}>
