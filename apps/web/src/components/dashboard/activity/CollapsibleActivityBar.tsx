...
export const CollapsibleActivityBar = React.memo<CollapsibleActivityBarProps>(
  ({ events, className = '', onEventClick, onViewAll }) => {
      ...
      >
        ...
        >
          <div className="flex items-center gap-3">
            ...
            <span className={`text-sm font-medium ${textColor}`}>Recent Activity</span>
            <span
              className={`
                ...
                {displayEvents.map((event) => (
                  ...
                  >
                    ...
                    <div className="flex items-center gap-2 mb-2">
                      <div
                        className={`w-2 h-2 rounded-full ${getCategoryColor(event.assetCategory)}`}
                      />
          ...
          <div className="px-4 pb-2.5 flex items-center gap-4 overflow-x-auto">
            {displayEvents.slice(0, 5).map((event, index) => (
              <div key={event.id} className="flex items-center gap-2 flex-shrink-0">
                <div
                  className={`w-1.5 h-1.5 rounded-full ${getCategoryColor(event.assetCategory)}`}
                />
                <span className={`text-xs ${textColor}`}>{event.assetNumber}</span>
                <span className={`text-xs ${mutedColor}`}>{formatTime(event.scannedAt)}</span>
