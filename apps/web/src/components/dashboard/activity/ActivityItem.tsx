...
export const ActivityItem = React.memo<ActivityItemProps>(
  ({ event, onClick, showLocation = true, isLast = false, animationDelay = 0 }) => {
    const { isDark } = useTheme();

    const timeAgo = useMemo(() => formatTimeAgo(event.scannedAt), [event.scannedAt]);
    const categoryConfig = useMemo(
      () =>
        CATEGORY_CONFIG[event.assetCategory as keyof typeof CATEGORY_CONFIG] ||
        CATEGORY_CONFIG.trailer,
      [event.assetCategory]
    );
      ...
      `}
        >
          <div className="flex items-start justify-between mb-1">
            ...
            <span className={`text-xs whitespace-nowrap ml-4 ${subtitleColor}`}>{timeAgo}</span>
          </div>
