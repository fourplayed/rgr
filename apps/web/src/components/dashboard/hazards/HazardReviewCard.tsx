...
export const HazardReviewCard = React.memo<HazardReviewCardProps>(
  ({ hazard, onReview, className = '', isDark = true }) => {
    ...
    return (
      <div
        ...
        style={{
          ...
          boxShadow: isDark ? '0 20px 40px rgba(0, 0, 0, 0.5)' : 'none', // No shadow on light theme to avoid corner artifacts
        }}
      >
        <div className="p-3">
          {/* Main Content Row */}
          ...
          {hazard.recommendedActions.length > 0 && (
            <div
              ...
              style={{ borderColor: isDark ? `${RGR_COLORS.chrome.medium}33` : '#9ca3af' }}
            >
              <button
                type="button"
