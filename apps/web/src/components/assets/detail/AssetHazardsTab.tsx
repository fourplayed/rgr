...
import { HazardSeverityLabels, HazardSeverityColors, HazardStatusLabels } from '@rgr/shared';
...
export const AssetHazardsTab = React.memo<AssetHazardsTabProps>(({ assetId, isDark }) => {
  ...
  if (alerts.length === 0) {
    return <div className={`text-center py-16 text-sm ${mutedColor}`}>No hazard alerts</div>;
  }
    ...
    <div className="flex flex-col h-full">
      <div className="flex-1 overflow-y-auto">
        {alerts.map((alert, i) => {
          ...
          return (
            <div key={alert.id} className={`px-5 py-3 ${i > 0 ? `border-t ${borderColor}` : ''}`}>
              <div className="flex items-start justify-between gap-2">
                <div className="flex items-center gap-2">
                  <AlertTriangle className="w-4 h-4" style={{ color: severityColor }} />
                  <span className={`text-sm font-medium ${textColor}`}>{alert.hazardType}</span>
                </div>
                {/* Severity badge */}
                <span
              ...
              <p className={`text-xs mt-1 ml-6 ${mutedColor} line-clamp-2`}>{alert.description}</p>
