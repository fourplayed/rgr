...
import type {
  ...
  MaintenanceListItem as MaintenanceListItemData,
  DefectReportListItem as DefectReportListItemData,
                ...
                <View style={styles.cardContentRow}>
                  <AppText style={styles.cardTitle} numberOfLines={1}>
                    {item.data.assetNumber
                      ? formatAssetNumber(item.data.assetNumber)
                      : 'Unknown Asset'}
                  </AppText>
          ...
          <View style={styles.cardRow}>
            ...
            <View style={styles.cardBody}>
              ...
              <View style={styles.scanFooter}>
                ...
                <AppText style={styles.scanTime}>{formatRelativeTime(item.data.createdAt)}</AppText>
              </View>
            </View>
          </View>
        </View>
...
export default function HomeScreen() {
  ...
  const { modal, closeModal, transitionTo, isTransitioning, handleExitComplete } =
    useModalTransition<HomeModalState>({ type: 'none' });
  ...
  const isRefetching =
    scansRefetching ||
    maintenanceRefetching ||
    defectsRefetching ||
    statsRefetching ||
    scanCountRefetching ||
    defectStatsRefetching;
  ...
  const statsCards = useMemo(
    () => [
      ...
      },
      {
        ...
        params: { status: 'serviced' },
      },
      {
        label: 'Reported Defects',
        value: defectStats?.reported ?? 0,
        color: colors.defectYellow,
        icon: 'warning' as const,
        route: '/(tabs)/maintenance' as const,
        params: { tab: 'defects', defectStatus: 'reported' },
      },
      {
        label: 'Out of Service',
        value: outOfServiceCount,
        color: colors.status.outOfService,
        icon: 'close-circle' as const,
        route: '/(tabs)/assets' as const,
        params: { status: 'out_of_service' },
      },
    ],
  ...
  const recentActivity = useMemo<DashboardActivityItem[]>(() => {
    const scanItems: DashboardActivityItem[] = (scans ?? []).map((s) => ({
      type: 'scan' as const,
      data: s,
      ...
      <>
        ...
        <View style={styles.profileSection}>
          <View style={styles.profileHeader}>
            <View>
              <Animated.Text
                style={[styles.greeting, { opacity: greetingOpacity, fontFamily: fonts.regular }]}
              >
                {greeting},
              </Animated.Text>
              <Animated.Text
                style={[styles.userName, { opacity: usernameOpacity, fontFamily: fonts.bold }]}
              >
                {user?.fullName}
              </Animated.Text>
            </View>
            <View style={{ position: 'absolute', top: 0, right: 0 }}>
              <Badge
                label={roleLabel}
                color={
                  colors.userRole[user?.role as keyof typeof colors.userRole] || colors.electricBlue
                }
                size="small"
              />
            </View>
